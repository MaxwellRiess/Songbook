...
function show(value) {
  ...
  const voicings = getVoicings(parsed.symbol);
  ...
  const shapes = voicings.length === 1 ? "1 shape" : `${voicings.length} shapes`;
  elements.summary.textContent = `${parsed.root} ${parsed.qualityName} · ${parsed.notes.join(" ")} · ${shapes}`;
